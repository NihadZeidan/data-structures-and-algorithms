...
const reversedString = (str) => {
    ...
    let toReverse = splitted.reduce((acc, val) => {
        return val + acc
    }, '')
...
const countNumberOfChildren = (arr) => {
    ...
    let total = arr.reduce((acc, obj) => {
        return (obj.children) ? acc + obj.children.length : acc
    }, 0)
...
const extractChildren = (arr) => {
    ...
    return filter.reduce((acc, obj) => {
        return (obj.children) ? acc.concat(obj.children) : acc
    }, []);
