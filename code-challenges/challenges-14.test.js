...
const screenForNames = (arr) => {
    ...
    let validator = /^\S(Mr. |Mrs. |Ms. |Dr. |)[\S\w]/g;
...
const toTitleCase = (arr) => {
    ...
    arr.forEach(str => {
        ...
        newArr.push(`${upper}${str.substring(1)}`);
...
let biggerThanLuke = (arr) => {
    ...
    let namesBiggerThanLuke = arr.filter(obj => {
    ...
    let onlyNames = namesBiggerThanLuke.map(obj => {
        return obj.name
    })
...
const detectTicTacToeWin = (board) => {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (board[i] == board[i][1] && board[i][1] == board[i][2] && !(board[i].includes(" "))) {
            return true;
        }
    }
