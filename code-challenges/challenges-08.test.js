...
const typeNum = (arr) => {
    let filterdNum = arr.filter(element => {
...
const containsAnd = (arr) => {
    let validator = /(and)/g;
    let filtered = arr.filter(string => {
...
const getStatName = (arr, minBaseStat) => {
    ...
    let prev = getBaseStatGreaterThan(arr, minBaseStat)

    return prev.map(obj => obj.stat.name);
...
const evenOddNumericValues = (arr) => {
    let toFilter = arr.filter(element => {
        if (typeof(element) === typeof(2)) {
            return element
        }
    })

    let strings = toFilter.map(num => {
