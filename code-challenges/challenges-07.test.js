...
const replaceZeros = (string) => {
    let validator = /[0]/g;
    return string.replace(validator, "zero");
}
...
const addQuestion = (arr) => {
    let questionMarkAdded = arr.map(value => {
        return value + "?"
    })
...
const evenOdd = (arr) => {
    ...
    let results = arr.map(number => {
        if (number % 2 === 0) {
            ...
            return 'odd'
        } else if (typeof(number) === typeof('g')) {
            return "N/A"
        } else {
            return "N/A"
        }
...
const extractAbilities = (arr) => {
    let abilities = arr.map(obj => {
        return Object.values(obj.ability)[1]
    })
    return abilities
};
