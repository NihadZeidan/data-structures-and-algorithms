...
const firstLetters = (arr) => {
    let firstLitter = arr.reduce((acc, val) => {
        acc.push(val.charAt(0));
        return acc
    }, [])
...
const findHappiness = (arr) => {
    let findHappy = arr.reduce((acc, val) => {

        val.includes(":)") ? acc.push(val) : null;
