...
const count = (target, input) => {
    let final = input.map(arr => {
        let toReduce = arr.reduce((acc, value) => {
            if (value === target) { acc++; }
            return acc
        }, 0)
        return toReduce
    })
    let sum = final.reduce((aCC, Value) => {
        aCC += Value;
        return aCC
    }, 0)
...
const totalSum = (input) => {
    let loopOverArrays = input.map(arr => {
        let sumEachArr = arr.reduce((acc, value) => {
            acc += value;
            return acc;
        }, 0)
        return sumEachArr;
    })
    let summing = loopOverArrays.reduce((Acc, Value) => {
        Acc += Value
        return Acc
    }, 0)
...
const divisibleByFiveTwoToThePower = (input) => {
    let outerArray = input.map(arr => {
        let onlyNumbers = arr.filter(element => {
            if (!(element % 5) && typeof(element) == typeof(6)) { return element };
        });
        // console.log(onlyNumbers);
        return onlyNumbers;
    })
    let powerTwo = outerArray.map(arr => {
        let eachArr = arr.map(number => {
...
let findMaleAndFemale = (data) => {
    let fitleration = data.filter(obj => {
        if (obj.gender === "male" || obj.gender === "female") { return obj }
    })
    let combainedNames = fitleration.reduce((acc, value) => {

        acc.push(value.name)
        return acc

    }, [])
    return combainedNames.join(' and ')
};
...
let findShortest = (data) => {
    let filteration = data.reduce((onlyTheName, value) => {

        onlyTheName.push(value.name);
        return onlyTheName;

    }, [])


    let lengthOfEachName = filteration.filter((element) => {
        if (element.length <= 5) { return element };
    });
    return lengthOfEachName[1];
};
